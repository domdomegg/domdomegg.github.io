...
[Next.js](https://nextjs.org/) is a React framework with a bunch of neat features to make web development easier.
...
My suspicion: 1 is likely fastest. However, 2 is often simpler - you don’t need to configure a second web server, and your deployment configuration can be the same across static and dynamic Next.js apps.

Knowing _how much slower_ this is lets us properly evaluate the penalty, and make evidence-based decisions about the trade-offs.

So, how fast is it? I took this blog (yes, the very one you’re reading) and tested how long it takes to load.
...
Wow, nginx is _fast_ - **4874x faster** than Next.js under heavy load for median response time. I had some sense of this before, but not quite how much it dominates on request duration. I think this is largely because Next.js slows down under heavy request load: its median time when processing a single request was much lower.

That said, the Next.js server is plenty fast for real-world use cases. It was just 20ms slower (a fifth of a blink) to load the page under low load, and even under heavy load (3733 requests/second) it still had a median request duration of 253ms.

The real-world penalty will likely be lower than this: network latency will start to dominate. And if you’ve got dynamic requests to APIs, those will almost certainly be the bottleneck rather than the static file serving.

Overall, with this small a penalty I’d pick running Next.js in most cases: it’s significantly easier to set up (provided you’re building a Next.js app already), and standardises your deployment across static and dynamic applications. It's probably only worth looking at nginx when you’re getting hundreds or thousands of requests per second, or if you've already got a really smooth nginx pipeline.
