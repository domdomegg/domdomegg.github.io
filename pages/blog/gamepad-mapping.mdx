...
I recently set out to make [a web game with controller support](https://github.com/domdomegg/controller-tutor/).

The [Gamepad API](https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API) lets you connect to controllers from the browser, giving you an array of buttons.

The specification defines a `standard` mapping from this array to the relevant buttons, and helpfully provides [an image](https://w3c.github.io/gamepad/standard_gamepad.svg) explaining it for something like a [DualShock controller](https://en.wikipedia.org/wiki/DualShock). But it's not immediately clear how this maps to an Xbox controller.
...
Finally, if you need to troubleshoot your controller, use [Human Benchmark's gamepad tester](https://hardwaretester.com/gamepad). I have no relation to this service other than as a user - I just found it really helpful.
