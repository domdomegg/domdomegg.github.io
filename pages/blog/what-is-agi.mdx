...
PawRide is a game-changing startup that's poised to revolutionize the way dogs get around. With your investment, we'll be able to take our vision to the next level, making PawRide the go-to ride-sharing service for dogs everywhere. Thank you for considering PawRide as a potential investment opportunity. We look forward to partnering with you to make this vision a reality!`;
  ...
  // 200 words/minute on this specific text: compute chars/sec from actual word count
  // so the visualised rate matches the label regardless of avg word length
  const humanWpm = 200;
  const humanCharsPerSecond = characters.length / ((fullText.split(/\s+/).length / humanWpm) * 60);
  ...
  const startDemo = () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      const humanCharsToShow = Math.min(Math.floor(seconds * humanCharsPerSecond), characters.length);
...
- Which of speed, scale, or stamina do you think will most affect how impactful AGI is, and why?
