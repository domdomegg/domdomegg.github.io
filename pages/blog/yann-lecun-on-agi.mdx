...
This is a summary of Yann LeCun's talk "Mathematical Obstacles on the Way to Human-Level AI". I've tried to make it accessible to people familiar with basic AI concepts, but not the level of maths Yann presents. You can watch the original talk [on YouTube](https://www.youtube.com/watch?v=ETZfkkv6V7Y).

[I disagree with Yann](/blog/yann-lecun-agi-disagreement/), but I have tried to represent his arguments as faithfully as possible. Understanding people who disagree with you is incredibly important for thinking properly about things.
...
LLMs are set up to predict the next token. This means predicting in a space with many options, of which only one is correct, and this scales exponentially with sequence length - so predictions are almost always incorrect. Similarly for images/videos: there are so many options, and the world is only partially predictable, that it's not feasible for the model to be correct.
...
AI systems spend the same amount of time on short problems and hard problems. They should work longer on hard problems.
- He thinks chain of thought is a trick that he implies isn't really solving it. (video timestamp: 19:33)
- Instead, AI systems should use optimization/search algorithms against an objective, rather than feed forward neural networks. The search process can then continue until it solves the problem - meaning it can work longer on a hard problem.
<Details title="LeCun's slide">
